...
召回仍是主要瓶颈。部分成功页正确性问题已修复，但既定权重和保留槽策略均未改善最终 macro F1，不进入 live capture；详见 `title-retention-offline-2026-08-09.md`。Gold 精确可用性聚合诊断已完成一次在线探针，但因 2 个 `integrity_failure` 而不完整：132/134 个唯一 work 可用，0 个精确未找到，实际 135 次 HTTP 尝试；推荐方向为 `null`。在完整性问题被离线复核前，不选择新数据源或其他提升方向，也不重跑在线探针。
