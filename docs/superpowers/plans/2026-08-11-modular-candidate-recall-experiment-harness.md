...
**Goal:** Implement Scheme B as a modular candidate-pool recall experiment harness in which a normal search-term method is added by Prompt/YAML or a manual action file, three retrieval action families are independently replaceable, historical fixed actions verify framework correctness, and only method-matched DeepSeek reproductions are compared with prior candidate-recall behavior.

**Architecture:** A read-only feasibility inventory runs before the framework build. A strict contract package then separates frozen inputs, a sealed Gold-document catalog, query generation, budgeted LLM calls, action validation/repair, retrieval handlers, replaceable search backends, versioned candidate-pool construction, optional post-pool stages, recall-only evaluation, artifact writing, and orchestration. Gold metadata is visible only while building an Oracle generation context; Gold identifiers and `IdentifierMap` enter only evaluator preflight. The runner iterates registered interfaces and contains no method-specific or action-specific branches.
...
- Automated implementation and verification are offline. Do not read `.env`, use real network, spend budget, mutate the ledger, rebuild frozen data, or rewrite historical runs/evidence. Any live canary or regenerated comparison is a later, separately authorized manual step.
- `snapshot_replay` is exact-request replay, not an arbitrary-query local search index. A novel Manual/DeepSeek action must fail with `snapshot_unavailable`; it may be retrieved only through an explicitly authorized `live_provider` or a future real `local_index`. Never fall back silently to live access.
- Oracle generation requires a hash-bound Gold document catalog. Every Gold association must have a non-empty title; abstract, authors, and publication year are supplied when available and their coverage is reported. It must not expose DOI, OpenAlex ID, Semantic Scholar ID, canonical ID, URLs, or provider request identifiers to DeepSeek.
...
- New-method candidate construction uses versioned policy `production-dedup-v1`, which calls `deduplicate_papers(papers, id_map=None)`. Exact historical replay may use the explicitly bound legacy `canonical-id-first-v1`. Candidate construction must not import or receive Gold or an evaluation identifier map.
- Only evaluator preflight may parse/adapt `paper_search.evaluation.dataset.IdentifierMap`; the input adapter only verifies its bytes and hash.
...
- Exact historical replay verifies harness correctness. Regenerated comparison additionally requires one reproduction recipe per historical method and aligned query IDs, visibility, Prompt method, model, temperature, action limits, budget, backend, denominator, and candidate-pool policy. Otherwise report `not_comparable` or `insufficient_historical_evidence`; never compare one generic Scheme B Prompt with all methods.
- A live regenerated comparison is a separately authorized step after all offline tasks pass. Three valid repeats may be scheduled within at most five attempts; every attempt retains artifacts, while infrastructure failures are excluded from valid-repeat pass/fail statistics.
...
configs/recall_experiments/
  ...
  methods/reproduce-<historical-method>.yaml  # only for inventory-proven reconstructable methods
...
configs/prompts/recall/
  ...
  scheme-b-blind.yaml
  reproduce-<historical-method>.yaml  # only for inventory-proven reconstructable methods
...
No method-specific standalone script is added. Historical reproduction files are declarative compatibility fixtures, not Python implementations. A future ordinary method adds only one method recipe and, for DeepSeek generation, one prompt artifact.

---

### Task 0: Inventory evidence, Gold metadata, and backend feasibility before the framework build

**Files:**
- Create: `src/paper_search/recall_experiments/__init__.py`
- Create: `src/paper_search/recall_experiments/inventory.py`
- Create: `tests/recall_experiments/test_inventory.py`
- Create: the five `configs/recall_experiments/historical/*.yaml` source bindings listed above
- Output only: `runs/_recall_history_inventory/source-inventory.json`

**Purpose:**

Make the go/no-go conditions visible before implementing the full harness. This task is deliberately small, read-only, offline, and reusable; it replaces late discovery that a historical method, Gold catalog, or snapshot cannot support the intended comparison.

- [ ] **Step 1: Write RED inventory tests**

Cover:

- every historical binding has explicit source paths, expected SHA-256, method identity, action family, evidence fields, and `candidate_pool_policy_version`;
- source statuses are exactly `exact | aggregate_only | insufficient | not_comparable`;
- the report distinguishes action evidence, exact Provider-response evidence, candidate-pool evidence, per-query Gold hits, aggregate metrics, Prompt/model/visibility metadata, and denominator evidence;
- the Gold association source and every candidate Gold-document source are separately inventoried;
- catalog coverage reports non-empty title coverage plus optional abstract/author/year coverage;
- each frozen request is classified `exact_snapshot_replay`; a novel request is classified `requires_live_provider_or_local_index`;
- duplicate, missing, changed-hash, or unbound files fail closed;
- no inventory function imports a live client, reads `.env`, opens the ledger, or mutates a historical path.

Run:

```powershell
& 'D:\AI Projects\Projects\.venv\Scripts\python.exe' -m pytest tests/recall_experiments/test_inventory.py -q
```

Expected: import failure for the missing inventory module.

- [ ] **Step 2: Bind and inspect the five historical sources**

Inventory Query Rewrite, LLM Query Variants, Query Evolution, Title Candidates, and Citation Expansion. Do not infer exact evidence from a method name or aggregate report. For each method record:
...
If the current workspace proves only some methods, record that result. No method may be silently omitted and no missing per-query evidence may be synthesized.

- [ ] **Step 3: Inventory Gold-document metadata producers**

Inspect the bound Gold association file and explicitly bound normalized `Paper`/Provider snapshot sources. Confirm that an association-only file such as `data/dev/gold.jsonl` is not treated as a document catalog. Report, per selected query, whether every Gold has a title and how many also have abstract, authors, and year.

If any selected Oracle Gold lacks a title, set `oracle_catalog_ready=false`. Do not fetch it in this task. The later Gold catalog builder may consume only these hash-bound sources; exact live metadata capture requires separate authorization.

- [ ] **Step 4: Implement the minimal read-only inventory command**

Add `python -m paper_search.recall_experiments.inventory ...` as a temporary module entry point or a pure function callable by tests. It may parse JSON/YAML and compute hashes, but it must not use Provider or LLM clients. Emit the report only under the new diagnostic output directory.

Run:

```powershell
& 'D:\AI Projects\Projects\.venv\Scripts\python.exe' -m paper_search.recall_experiments.inventory --config-root configs/recall_experiments/historical --out runs/_recall_history_inventory
& 'D:\AI Projects\Projects\.venv\Scripts\python.exe' -m pytest tests/recall_experiments/test_inventory.py -q
& 'D:\AI Projects\Projects\.venv\Scripts\python.exe' -m ruff check src/paper_search/recall_experiments/inventory.py tests/recall_experiments/test_inventory.py
& 'D:\AI Projects\Projects\.venv\Scripts\python.exe' -m mypy src/paper_search/recall_experiments/inventory.py
```

- [ ] **Step 5: Apply the continuation decision before Task 1**

Record one of these outcomes in `source-inventory.json`:

- `core_framework_ready`: at least one exact historical action/response source can exercise the framework;
- `overall_compatibility_ready`: in addition, at least two exact sources exist, including one text and one non-text family;
- `core_framework_blocked`: there is no exact request/response source to verify the generic path;
- `oracle_catalog_blocked`: selected Oracle queries lack title coverage.

`core_framework_ready` is sufficient to continue building reusable modules, but only `overall_compatibility_ready` can later support an overall Scheme B historical-compatibility `passed` conclusion. `oracle_catalog_blocked` permits fixed replay work but blocks Oracle DeepSeek execution until a separately authorized catalog capture is sealed.

- [ ] **Step 6: Commit the inventory module, tests, and bindings**

```powershell
git add -- src/paper_search/recall_experiments/__init__.py src/paper_search/recall_experiments/inventory.py tests/recall_experiments/test_inventory.py configs/recall_experiments/historical
git diff --cached --check
git commit -m "feat: inventory candidate recall evidence"
```

Do not stage `runs/_recall_history_inventory` or any pre-existing untracked file.
...
**Files:**
- Modify: `src/paper_search/recall_experiments/__init__.py`
...
- `GoldDocument(title, abstract | None, authors, publication_year | None, metadata_coverage)` with no identifier fields; title is non-empty, optional fields preserve all available catalog content
...
- `RetrievalExecutionContext(query_id, provider_filters, max_results_per_action, seed_candidates)`; it has no Gold, `IdentifierMap`, recall, or prior-hit fields
...
The scanner rejects forbidden keys and identifier/URL patterns such as DOI syntax, OpenAlex/S2 IDs or URLs, canonical IDs, and provider request IDs. It must not reject an ordinary title or abstract merely because its prose mentions “OpenAlex” or “Semantic Scholar”.
...
- `RetrievalRecipe`
- `CandidatePoolRecipe(policy_version: production-dedup-v1 | canonical-id-first-v1)`
- `EvaluationRecipe`
- `SampleBinding`
...
Require strict unknown-field rejection, safe relative paths, hashable canonical serialization, and these recipe-local cross-field rules:
...
- new methods default to `candidate_pool.policy_version=production-dedup-v1`; a historical recipe may use `canonical-id-first-v1` only when its binding proves that legacy policy.

The recipe loader cannot prove whether Blind and Oracle sample IDs overlap because the sample is a separate artifact. Test that this and Gold-versus-seed isolation are rejected by the combined preflight after recipe and dataset loading, not by `load_recall_recipe()` alone.
...
The live backend declaration does not authorize a call. `prepare-context` and `validate-actions` remain offline; `run` refuses unless the caller supplies the separate authorization mechanism. Do not set `snapshot_replay` for novel manual actions, because an exact-request replay backend cannot answer them.
...
- Create: `src/paper_search/recall_experiments/inputs/gold_catalog.py`
...
- `GoldDocumentCatalogBuilder.build(gold_associations, bound_paper_sources) -> SealedGoldDocumentCatalog`
- `GoldDocumentCatalogSource.load(binding) -> SealedGoldDocumentCatalog`
...
- combined preflight, not the input adapter alone, rejects overlapping Oracle and Blind sample partitions;
...
- Oracle fails before generation if any selected Gold lacks a non-empty title; missing abstract, authors, or publication year is allowed, preserved as null/empty, and reflected in `metadata_coverage`;
- Blind loads and evaluates Gold IDs but never loads Gold-document contents.
- the generation-facing `GoldDocument` contains every available title/abstract/author/year field but no private catalog join keys or identifiers;
...
Treat the Gold-document catalog as a separately replaceable, hash-bound input. The sample binding stores a path and SHA-256 validated by the existing `Sha256` type and recomputed from exact bytes. Do not assume `data/recall_freeze/dev-gold-documents.jsonl` already exists.

Implement the producer in `gold_catalog.py`. It joins private Gold associations to explicitly bound, hash-verified normalized `Paper` records or Provider snapshots identified by Task 0. The sealed private rows may contain `query_id` and Gold evaluation ID for evaluator association, but `to_generation_documents()` strips all IDs, URLs, request IDs, and Provider-specific keys.

Build from existing frozen sources first and emit a deterministic catalog plus source manifest. Missing entries remain explicit; the builder never performs network access. If title coverage is incomplete, Oracle CLI preflight returns `oracle_catalog_incomplete` and no DeepSeek request is allowed. A later exact metadata capture may fill the missing rows only after separate network/fee authorization and must use existing live Provider, budget, snapshot, and ledger mechanisms before resealing the catalog.
...
This three-query sample is a diagnostic canary only; it is sufficient for text/title/manual workflow checks but cannot support a performance or generalization claim. Citation integration uses exact non-Gold seed candidates bound by the historical Citation Expansion source inventoried in Task 0 and normalized later in Task 11. If that evidence lacks a legitimate frozen seed, Citation Expansion is reported `insufficient_historical_evidence` and remains covered by synthetic handler/backend tests; do not substitute a Gold paper, live lookup, or dynamically retrieved text/title result.
...
```powershell
git commit -m "feat: add frozen recall inputs and gold catalog"
```
...
### Task 4: Add replaceable LLM/search backends and an explicit action registry

**Files:**
- Create: `src/paper_search/recall_experiments/generation/backends.py`
...
- Create: `tests/recall_experiments/test_llm_backend.py`
...
- `LLMBackend.generate(request, call_kind) -> LLMBackendResult`
- `BudgetedLLMBackend`
...
Search backend tests must prove one reservation per logical call, correct settle/fail/release behavior, structured infrastructure failure classification, and exact propagation of normalized `Paper`, usage, provenance, and error codes. They must also prove that `ReplaySearchProvider` returns `snapshot_unavailable` for a novel request and never falls back to live access.

LLM backend tests use a fake analyzer with the same `BudgetReservation` requirement as the production analyzer. Prove that:

- each initial generation receives its own reservation;
- each repair receives a second, independent reservation;
- every reservation reaches one terminal usage/fail/release state and is represented in snapshots/ledger events;
- analyzer `errors[0].code == "invalid_json"` is returned as a repairable generation error;
- provider/auth/rate/network/snapshot/accounting failures are `infrastructure_failure` and do not consume a semantic repair count.

Run `python -m pytest tests/recall_experiments/test_retrieval_registry.py tests/recall_experiments/test_llm_backend.py -q`. Expected: import failure for the missing registry/backend modules.
...
`BudgetedLLMBackend` wraps the existing analyzer/live/replay adapters and owns the `BudgetReservation` required by analyzer calls. It exposes a small result contract to `DeepSeekPromptGenerator`; the generator never constructs reservations or writes directly to the ledger.
...
```powershell
& 'D:\AI Projects\Projects\.venv\Scripts\python.exe' -m pytest tests/recall_experiments/test_retrieval_registry.py tests/recall_experiments/test_llm_backend.py tests/unit/test_retrieval_snapshot_adapters.py tests/unit/test_citation_expand.py -q
& 'D:\AI Projects\Projects\.venv\Scripts\python.exe' -m ruff check src/paper_search/recall_experiments/generation/backends.py src/paper_search/recall_experiments/retrieval/backends.py src/paper_search/recall_experiments/retrieval/registry.py tests/recall_experiments/test_retrieval_registry.py tests/recall_experiments/test_llm_backend.py
& 'D:\AI Projects\Projects\.venv\Scripts\python.exe' -m mypy src/paper_search/recall_experiments/generation/backends.py src/paper_search/recall_experiments/retrieval/backends.py src/paper_search/recall_experiments/retrieval/registry.py
git add -- src/paper_search/recall_experiments/generation/backends.py src/paper_search/recall_experiments/retrieval/__init__.py src/paper_search/recall_experiments/retrieval/backends.py src/paper_search/recall_experiments/retrieval/registry.py tests/recall_experiments/test_retrieval_registry.py tests/recall_experiments/test_llm_backend.py
git diff --cached --check
```
...
```powershell
git commit -m "feat: add budgeted recall backends"
```
...
Assert that `TitleSearchHandler` passes the already validated complete `title_text` as a normal provider search query, uses the same replaceable backend, and preserves provider rank. It must not call `extract_title_candidates`: that function extracts a list from raw LLM output, while NFKC/whitespace normalization and strict title validation already occurred in the generator/validator layer. Tests and docstrings must not claim an exact-title endpoint or exact-title match—the existing `LLMTitleCandidateStage` calls `provider.search(title, {}, limit, reservation)`.
...
Keep only title-search execution behavior in this module; shared action validation remains in `validation.py`. Do not import `text_search.py`. Reuse the existing title stage only as a reference for Provider call and provenance behavior. Verify it before starting citation work:
...
- every successful result includes the selected frozen seed plus expanded neighbors, with distinct `seed` and `expanded` provenance, matching the historical “prior candidates + additions” pool meaning.
...
### Task 6: Build versioned raw candidate pools and the empty future-stage pipeline
...
**Interfaces:**
- `CandidatePoolBuilder(policy_version).build(query_id, action_results) -> CandidatePool`
...
Cover stable action/result/rank order, preservation of every source evidence record, empty results, partial errors, and identical output when result order is repeated. Add explicit fixtures for both locked policies:

- `production-dedup-v1`: DOI/external-ID/exact-title/fuzzy-title duplicate merging through existing `deduplicate_papers`;
- `canonical-id-first-v1`: the legacy Query Evolution projection that keeps the first occurrence of each canonical ID without production fuzzy merging.

Reject unknown policy versions. Assert the selected policy version is serialized into candidate-pool and report artifacts.
...
For `production-dedup-v1`, flatten `Paper` objects in registered action order and call:

```python
deduplicated = deduplicate_papers(flattened, id_map=None)
```

For `canonical-id-first-v1`, implement only the explicitly tested legacy first-by-canonical-ID projection. Then attach all source evidence by duplicate-cluster membership. Do not apply `QuerySpec` hard filters, ranking, or limits after collection.

New method recipes must use `production-dedup-v1`; the legacy policy is selected only by a hash-bound historical source recipe. The evaluator later marks differently projected results `not_comparable` unless it is given an explicit matched projection.
...
- terminal conclusions `passed | failed | not_comparable | insufficient_valid_repeats | insufficient_historical_evidence`
...
`preflight()` is the only place that calls `IdentifierMap.from_bytes`. After recipe and dataset are both loaded, it rejects duplicate resolved Gold associations, overlapping Oracle/Blind query IDs, any `seed_candidate` that resolves to a Gold ID, mismatched denominators, and an Oracle association without a title in the sealed catalog. It then returns separate generation-safe query contexts and private scoring data. The runner passes only generation-safe contexts to generators/handlers/pool construction.
...
Require the historical binding and replay pool to use the same locked `candidate_pool_policy_version`. A regenerated comparison must also align that policy or provide both explicit projections; otherwise return `not_comparable` before calculating tolerance deltas.
...
Assign every scheduled execution `attempt_id=attempt-01..attempt-05`. A successful semantic result receives `valid_repeat_ordinal=1..3`; an infrastructure failure has `valid_repeat_ordinal=null`. Retain every attempt in the report and exclude infrastructure failures only from valid-repeat pass/fail statistics. Require three valid repeats within five scheduled attempts and at least two passing repeats. Report min/median/max for hits and macro recall. If fewer than three valid repeats exist after five attempts, return `insufficient_valid_repeats` without a sixth attempt.
...
Require exact query coverage, no unknown IDs, one immutable action batch per query/attempt, strict validation before retrieval, and byte-identical fixed replay. Manual generation reads a user-prepared JSON artifact; it performs no LLM call.
...
Both generators return the same `GenerationResult`. Write `generation/<attempt_id>/<query_id>.json` before calling any handler. Once written, the runner cannot replace the action batch based on retrieval results.
...
generation/<attempt_id>/<query_id>.json
retrieval/<attempt_id>/<query_id>.json
candidate-pools/<attempt_id>/<query_id>.json
recall-report.json
```

Use `generation/attempt-01/...` as well; `<repeat>` is not a directory name. Every artifact and report entry records `attempt_id`, `attempt_status`, and nullable `valid_repeat_ordinal`. Failed attempts are never overwritten or renumbered after later success.
...
- Create when reconstructable: `configs/recall_experiments/methods/reproduce-<historical-method>.yaml`
- Create when reconstructable: `configs/prompts/recall/reproduce-<historical-method>.yaml`
...
Use a recording fake `LLMBackend`. Oracle payload must include only query data, seed data, allowed action schema, and every available Gold title/abstract/authors/year field plus coverage flags. Recursively reject forbidden identifier keys and identifier/URL patterns: DOI syntax, OpenAlex/S2 IDs or URLs, canonical IDs, provider request IDs, prior Gold hits, and evaluation results. Include a legitimate paper title/abstract that mentions “OpenAlex” or “Semantic Scholar” and prove it is retained.
...
Parameterize all structured validation failures from Task 1, including analyzer `invalid_json`. The first invalid response triggers exactly one repair call containing error codes, field paths, previous output, and the allowed change scope. A second invalid response returns `generation_failure`. Assert that initial and repair calls have separate backend call IDs/reservations. Provider/auth/rate-limit/network/snapshot/accounting errors return `infrastructure_failure` and do not consume the semantic repair attempt.

- [ ] **Step 3: Implement through existing LLM adapters**

Inject the `LLMBackend` from Task 4; do not inject a raw analyzer and do not instantiate `OpenAICompatibleLLMClient` here. The backend adapts `LiveCaptureLLMAnalyzer` or `ReplayLLMAnalyzer` and owns their `BudgetReservation`. Bind prompt bytes and SHA-256 into snapshot identity. Keep temperature `0`, model `deepseek-v4-flash`, and `repair_attempts=1` locked by recipe unless a hash-bound historical reproduction proves a different original setting.

- [ ] **Step 4: Add generic exploration and per-method reproduction recipes**

Keep `scheme-b-oracle.yaml` and `scheme-b-blind.yaml` as new exploratory methods. Separately, consume Task 0 inventory and create one reproduction recipe/Prompt for each historical method whose generation method is actually reconstructable. Each locks the original query IDs, visibility, Prompt method, model, temperature, action limits, retrieval budget/backend, Gold denominator, and candidate-pool policy.

If a method did not use DeepSeek or any required element is unknown, do not invent a generic substitute. Mark its historical binding `generation_comparison=not_comparable` or `insufficient_historical_evidence` with the missing fields. Fixed-action replay remains eligible independently.
...
git add -- src/paper_search/recall_experiments/generation/deepseek.py tests/recall_experiments/test_generation.py configs/prompts/recall configs/recall_experiments/methods
git diff --cached --check
```
...
### Task 10: Add composition and a split offline/live CLI for fast feasibility testing
...
- fixed historical recipes use exact replay; novel Manual/DeepSeek actions are rejected by replay with `snapshot_unavailable` and require an explicitly authorized live backend or future local index;
- live generation/search requires `--allow-live` and an authorized recipe/backend combination; there is no implicit fallback in either direction;
- `prepare-context`, `validate-actions`, inventory, and exact replay never open `.env`, ledger, or network;
...
- [ ] **Step 3: Verify offline preparation separately from live candidate recall**
...
Expected offline workflow: the user can copy the prepared Oracle context to DeepSeek, paste returned actions into one JSON file, and validate/freeze them without writing a new script or enabling an LLM client in the harness. This proves only context construction and action validity, not candidate recall.

First prove that trying those novel actions with `snapshot_replay` returns `snapshot_unavailable`. Then, only in the separately authorized live phase, run:

```powershell
paper-search recall run --recipe configs/recall_experiments/methods/manual-oracle-smoke.yaml --sample configs/recall_experiments/samples/dev-smoke-3.yaml --actions runs/recall_inputs/manual-oracle-smoke/actions.json --allow-live --out runs/_recall_smoke_manual_oracle_live
```

This three-query live run is a diagnostic canary. It tests candidate recall for the supplied actions, but it is not an offline test and does not establish method performance.

If required Gold metadata is absent, or a fixed replay request lacks an exact response, stop with the corresponding preflight code; do not switch to live automatically.
...
- Modify only if Task 0 found a binding error: the five `configs/recall_experiments/historical/*.yaml` bindings
...
- [ ] **Step 1: Consume and re-verify the Task 0 inventory**

Load `runs/_recall_history_inventory/source-inventory.json`. Before any replay, recompute every bound historical source SHA-256 and require exact equality with both the binding and the Task 0 pre-run inventory. Do not scan arbitrary run directories and guess identities.

The consumed record includes:
...
For `evidence_level=exact`, replay the exact historical actions against exact frozen responses using the bound candidate-pool policy, then require exact normalized candidate-ID sets, per-query hits, total Gold associations, and macro candidate recall. For `aggregate_only`, compare only the stored aggregate fields and label per-query equality `not_provable`. Any mismatch blocks regenerated comparison for that method.

All five bindings must reach one explicit terminal state:

- `exact_replay_passed`; or
- `aggregate_only` with the unprovable fields recorded;
- `not_comparable` with the semantic mismatch recorded; or
...
paper-search recall inventory-history --config-root configs/recall_experiments/historical --out runs/_recall_history_inventory_after
...
Compare every `source_hashes` entry in `_recall_history_inventory_after/source-inventory.json` with Task 0's inventory. This hash comparison, not `git diff`, proves that tracked, untracked, or ignored historical inputs are byte-identical. Inventory/compare artifacts remain untracked diagnostic outputs. Then commit only adapters, tests, and any corrected binding:
...
```powershell
& 'D:\AI Projects\Projects\.venv\Scripts\python.exe' -m pytest tests/recall_experiments/test_inventory.py tests/recall_experiments/test_contracts.py tests/recall_experiments/test_recipes.py tests/recall_experiments/test_inputs.py -q
& 'D:\AI Projects\Projects\.venv\Scripts\python.exe' -m pytest tests/recall_experiments/test_llm_backend.py tests/recall_experiments/test_retrieval_registry.py tests/recall_experiments/test_text_search.py tests/recall_experiments/test_title_search.py tests/recall_experiments/test_citation_expand.py -q
...
- novel actions cannot be executed by exact snapshot replay and no backend silently falls back to live access;
- every LLM initial/repair call has an independent budget terminal state;
- historical source hashes exactly match both the binding and Task 0 inventory, covering tracked, untracked, and ignored sources;
- user-owned paths listed in Global Constraints retain their pre-task status and content.
...
### Task 13: Execute the separately authorized live canary and method-matched regenerated comparisons

**Preconditions:**

- Tasks 0–12 are green.
- Oracle Gold-document catalog is hash-bound and every selected Gold has a title; optional metadata coverage is recorded.
- Each compared historical method has `exact_replay_passed` and an inventory-proven reproduction recipe.
- For each comparison, the same query IDs, Gold denominator, Gold visibility, Prompt method, model, temperature, action limits, search budget/backend, and candidate-pool policy are locked.
...
Run `prepare-context` first and inspect the sanitized Oracle payload. DeepSeek may generate the actions outside the harness and the user may paste them into `ManualActionGenerator`, or the budgeted DeepSeek generator may be used directly. Validate/freeze the action artifact before retrieval, then run one explicitly authorized live attempt on `dev-smoke-3.yaml`. Verify generation JSON, repair count, retrieval provenance, candidate-pool contents, usage settlement, snapshot seal, and replay of the newly captured exact requests before expanding the sample.

The canary asks only whether the end-to-end path and candidate recall are obviously viable on three selected queries. It does not decide Scheme B performance, compatibility, or generalization.

- [ ] **Step 2: Schedule up to five attempts to obtain three valid repeats**

Run this schedule separately for every inventory-proven reproduction recipe. For each scheduled attempt, freeze generated actions before retrieval and write artifacts under `attempt-01` through `attempt-05`. Record `attempt_status`; assign `valid_repeat_ordinal=1..3` only to valid semantic results. Do not adjust search terms after seeing results. Classify provider/auth/rate-limit/snapshot/ledger failures as `infrastructure_failure`; retain their artifacts, exclude them from valid-repeat pass/fail statistics, and count them toward the five scheduling attempts.

- [ ] **Step 3: Compare each reproduction only with its matched historical method**

Use only matched query sets, denominators, generation identities, backends, budgets, and candidate-pool policies. A repeat passes its matched method when Gold count delta is at most `±1`, macro candidate recall delta at most `0.02`, and historical Gold retention at least `0.90`. Require at least two passing repeats among three valid repeats.

Do not compare the generic `scheme-b-oracle` or `scheme-b-blind` output with all five historical methods. Those recipes remain exploratory. A historical method whose original generation settings cannot be reconstructed remains `not_comparable` or `insufficient_historical_evidence`, even if fixed replay passed.

The final report must separate:

- harness exact-replay correctness;
- method-matched regenerated compatibility per historical method;
...
- [ ] Manual DeepSeek output can be prepared and validated offline, then candidate recall can be tested on three frozen queries only through a separately authorized live backend or future arbitrary-query local index.
- [ ] Oracle sees every Gold title and all available abstract/author/year content but no Gold identifiers; coverage is reported, and Blind receives no Gold document field.
...
- [ ] Candidate pool is complete, policy-versioned, provenance-preserving, unfiltered, unranked, and Gold-independent.
...
- [ ] DeepSeek generation uses one structured repair attempt, independent initial/repair budget reservations, and no result-driven prompt rewrite.
...
- [ ] Method-matched compatibility uses the approved `±1`, `0.02`, `0.90`, and `2/3` thresholds; a generic Scheme B Prompt is not compared with unrelated historical methods.
...
- [ ] Inventory runs before the full build and states the available historical evidence, Gold catalog coverage, backend feasibility, and maximum defensible conclusion.
- [ ] Gold title completeness and optional-field coverage are checked before DeepSeek construction; no identifier can enter its payload.
- [ ] Exact snapshot replay rejects novel requests and never falls back silently to live access.
- [ ] Attempt identity is distinct from valid-repeat ordinal, and failed attempt artifacts remain auditable.
