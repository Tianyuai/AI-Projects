...
def _manifest(run_id: str = "formal-1") -> RunManifest:
    return RunManifest(
        ...
        identifier_map_sha256=SHA_A,
        source_git_sha="abc1234",
        tracked_source_dirty=False,
        config_hash=SHA_A,
        input_lock_sha256=SHA_A,
        prompt_version="query-analyze-v1",
        snapshot_set_id=SHA_B,
...
def _workspace(tmp_path: Path, **kwargs: object) -> FormalRunWorkspace:
    return FormalRunWorkspace(
        runs_root=tmp_path / "runs",
        manifest=_manifest(),
        input_lock_bytes=b"exact: input-lock\n",
...
def _replay_lock() -> ReplayLock:
    candidate = CandidateLock.model_validate(
...
def test_workspace_stages_exact_lock_and_ordered_jsonl_tree(tmp_path: Path) -> None:
    ...
    assert (workspace.work_dir / "config.lock.yaml").read_bytes() == b"exact: input-lock\n"
...
def test_complete_status_is_independent_from_failed_gate(tmp_path: Path) -> None:
    ...
    assert set(path.name for path in destination.iterdir()) == {
        ...
        "metrics.json", "usage.json", "failures.jsonl",
    }
